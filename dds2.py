...
running = True
packets_per_two_seconds = 2000 # Default rate (1000/sec), can be changed by --rate
...
def get_session(target_url):
    with counter_lock:
        ...
        if connection_reset_counter.get(target_url, 0) > 20000: reset_connection_pool(target_url)
...
def make_request(target_url, thread_id):
    ...
    try:
        ...
        for attempt in range(retries):
            ...
            except requests.exceptions.Timeout: logger.warning(f"{target_url} Timeout (Att {attempt+1})"); real_success, report_success = False, is_forced_active; continue # Continue to next attempt on timeout unless last
            except requests.exceptions.ConnectionError as e: logger.warning(f"{target_url} ConnectionError: {e} (Att {attempt+1})"); with counter_lock: connection_reset_counter[target_url] = connection_reset_counter.get(target_url, 0) + 10; real_success, report_success = False, is_forced_active; continue # Continue unless last
            except Exception as e: logger.error(f"T{thread_id} Error {target_url}: {e}", exc_info=False); with counter_lock: thread_errors[target_url] = thread_errors.get(target_url, 0) + 1; real_success, report_success = False, is_forced_active; continue # Continue unless last
